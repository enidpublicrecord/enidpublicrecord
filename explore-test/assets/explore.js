(()=>{'use strict';
const BUILD='explore-enid-v2.0-staging-20260922';let promptEvent=null;
...
privacy:'<p>No login is required. Explore Enid is designed to avoid unnecessary collection of personal data. This staging build does not include the production analytics tag.</p>',
...
version:`<p>${BUILD}</p><p>STAGING preview · noindex.</p>`
};sub(b.textContent.trim(),c[k]||'')});
window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();promptEvent=e});
async function installExplore(){if(promptEvent){promptEvent.prompt();await promptEvent.userChoice;promptEvent=null}else{sub('Install Explore Enid','<p>On supported browsers, use the browser’s Install app or Add to Home screen option. This staging build uses its own /explore-test/ scope and will not replace the production Explore Enid app.</p>')}}
