...
const CACHE='epr-water-v1.4.2-app-shell-20260921';
const SHELL=[
  '/water/','/water/index.html','/water/assets/water.css','/water/assets/water.js',
  '/water/manifest.webmanifest?v=9.2-prod','/water/assets/epr-logo.png','/water/assets/epr-app-icon-v2-180.png','/water/assets/epr-app-icon-v2-192.png','/water/assets/epr-app-icon-v2-512.png'
];
const DATA=[
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('epr-water-')&&!k.startsWith('epr-water-test-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
